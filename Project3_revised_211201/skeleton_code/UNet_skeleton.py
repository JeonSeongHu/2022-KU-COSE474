...
class Unet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat([x, conv4], dim=1) ####### hint : concatenation (Lecture slides)
        x = self.convUp4(x)
        x = self.upsample(x)
        print(x.shape, conv3.shape)
        x = torch.cat([x, conv3],dim=1) ####### hint : concatenation (Lecture slides)
        x = self.convUp3(x)
        x = self.upsample(x)
        x = torch.cat([x, conv2], dim = 1) ####### hint : concatenation (Lecture slides)
        x = self.convUp2(x)
        x = self.upsample(x)
        x = torch.cat([x, conv1], dim = 1) ####### hint : concatenation (Lecture slides)
        x = self.convUp1(x)
